...
import type { DataPublicPluginStart, IndexPatternAggRestrictions, SavedQuery } from '@kbn/data-plugin/public';
...
import type { Datatable, ExpressionAstExpression, IInterpreterRenderHandlers } from '@kbn/expressions-plugin/common';
import type { UiActionsStart, VisualizeFieldContext } from '@kbn/ui-actions-plugin/public';
import type { CellValueContext, EmbeddableEditorBreadcrumb, EmbeddableEditorState, EmbeddableStateTransfer } from '@kbn/embeddable-plugin/public';
import type { SavedObjectsResolveResponse } from '@kbn/core-saved-objects-api-server';
import type { BrushTriggerEvent, ChartsPluginSetup, ClickTriggerEvent } from '@kbn/charts-plugin/public';
...
import type { ApplicationStart, ChromeStart, CoreStart, DocLinksStart, ExecutionContextStart, HttpStart, IUiSettingsClient, NotificationsStart, OverlayRef } from '@kbn/core/public';
...
import type { DimensionLink, LensConfiguration, LensEditEvent, LensEditSupportedActions, LensLayerType, LensTableRowContextMenuEvent, Visualization, VisualizationState } from './visualizations/types';
...
export interface ILensDocumentService {
    ...
    checkForDuplicateTitle: (options: CheckDuplicateTitleOptions, onTitleDuplicate: () => void) => Promise<boolean>;
}
export interface LensAttributesService {
    ...
    saveToLibrary: (attributes: LensSavedObjectAttributes, references: Reference[], savedObjectId?: string) => Promise<string>;
    checkForDuplicateTitle: (props: CheckDuplicateTitleProps) => Promise<{
        isDuplicate: boolean;
    }>;
}
...
export type StartServices = Pick<CoreStart, 'overlays' | 'analytics' | 'i18n' | 'theme' | 'userProfile'>;
...
export type FieldOnlyDataType = 'document' | 'ip' | 'histogram' | 'geo_point' | 'geo_shape' | 'counter' | 'gauge' | 'murmur3';
...
type UserMessageDisplayLocation = {
    id: 'toolbar' | 'embeddableBadge' | 'visualization' | 'visualizationOnEmbeddable' | 'visualizationInEditor' | 'textBasedLanguagesQueryInput' | 'banner';
} | {
    id: 'dimensionButton';
    dimensionId: string;
};
...
export type UserMessagesGetter = (locationId: UserMessagesDisplayLocationId | UserMessagesDisplayLocationId[] | undefined, filters?: UserMessageFilters) => UserMessage[];
...
export interface EditorFrameSetup {
    registerDatasource: <T, P, Q>(datasource: Datasource<T, P, Q> | (() => Promise<Datasource<T, P, Q>>)) => void;
    registerVisualization: <T, P, ExtraAppendLayerArg>(visualization: Visualization<T, P, ExtraAppendLayerArg> | (() => Promise<Visualization<T, P, ExtraAppendLayerArg>>)) => void;
}
...
export type StateSetter<T, OptionsShape = unknown> = (newState: StateSetterArg<T>, options?: OptionsShape) => void;
...
export interface Datasource<T = unknown, P = unknown, Q = Query | AggregateQuery> {
    id: string;
    alias?: string[];
    initialize: (state?: P, references?: Reference[], initialContext?: VisualizeFieldContext | VisualizeEditorContext, indexPatternRefs?: IndexPatternRef[], indexPatterns?: IndexPatternMap) => T;
    getPersistableState: (state: T) => {
        state: P;
        references: Reference[];
    };
    insertLayer: (state: T, newLayerId: string, linkToLayers?: string[]) => T;
    createEmptyLayer: (indexPatternId: string) => T;
    removeLayer: (state: T, layerId: string) => {
        newState: T;
        removedLayerIds: string[];
    };
    clearLayer: (state: T, layerId: string) => {
        newState: T;
        removedLayerIds: string[];
    };
    cloneLayer: (state: T, layerId: string, newLayerId: string, getNewId: (id: string) => string) => T;
    ...
    initializeDimension?: (state: T, layerId: string, indexPatterns: IndexPatternMap, value: {
        columnId: string;
        groupId: string;
    ...
    LayerSettingsComponent?: (props: DatasourceLayerSettingsProps<T>) => React.ReactElement<DatasourceLayerSettingsProps<T>> | null;
    ...
    DimensionEditorComponent: (props: DatasourceDimensionEditorProps<T>) => ReactElement<DatasourceDimensionEditorProps<T>> | null;
    LayerPanelComponent: (props: DatasourceLayerPanelProps<T>) => JSX.Element | null;
    getDropProps: (props: GetDropPropsArgs<T>) => {
        dropTypes: DropType[];
        nextLabel?: string;
    } | undefined;
    onDrop: (props: DatasourceDimensionDropHandlerProps<T>) => T | undefined;
    getCustomWorkspaceRenderer?: (state: T, dragging: DraggingIdentifier, indexPatterns: Record<string, IndexPattern>) => undefined | (() => JSX.Element);
    ...
    onIndexPatternChange?: (state: T, indexPatterns: IndexPatternMap, indexPatternId: string, layerId?: string) => T;
    onIndexPatternRename?: (state: T, oldIndexPatternId: string, newIndexPatternId: string) => T;
    triggerOnIndexPatternChange?: (state: T, oldIndexPatternId: string, newIndexPatternId: string) => void;
    onRefreshIndexPattern: () => void;
    toExpression: (state: T, layerId: string, indexPatterns: IndexPatternMap, dateRange: DateRange, nowInstant: Date, searchSessionId?: string, forceDSL?: boolean, projectRouting?: ProjectRouting) => ExpressionAstExpression | string | null;
    getDatasourceSuggestionsForField: (state: T, field: unknown, filterFn: (layerId: string) => boolean, indexPatterns: IndexPatternMap) => Array<DatasourceSuggestion<T>>;
    getDatasourceSuggestionsForVisualizeCharts: (state: T, context: NavigateToLensContext['layers'], indexPatterns: IndexPatternMap) => Array<DatasourceSuggestion<T>>;
    getDatasourceSuggestionsForVisualizeField: (state: T, indexPatternId: string, fieldName: string, indexPatterns: IndexPatternMap) => Array<DatasourceSuggestion<T>>;
    getDatasourceSuggestionsFromCurrentState: (state: T, indexPatterns?: IndexPatternMap, filterFn?: (layerId: string) => boolean, activeData?: Record<string, Datatable>) => Array<DatasourceSuggestion<T>>;
     ...
     */
    getSearchWarningMessages?: (state: P, warning: SearchResponseWarning, request: estypes.SearchRequest, response: estypes.SearchResponse) => UserMessage[];
    /**
     ...
     */
    isEqual: (persistableState1: P, references1: Reference[], persistableState2: P, references2: Reference[]) => boolean;
    /**
    ...
    getDatasourceInfo: (state: T, references?: Reference[], dataViewsService?: DataViewsPublicPluginStart) => Promise<DataSourceInfo[]>;
...
export interface DatasourceDataPanelProps<T = unknown, Q = Query | AggregateQuery> {
    ...
    showNoDataPopover: () => void;
    core: Pick<CoreStart, 'http' | 'notifications' | 'uiSettings' | 'overlays' | 'theme' | 'application' | 'docLinks'>;
    query: Q;
    dateRange: DateRange;
    filters: Filter[];
...
export type DatasourceDimensionEditorProps<T = unknown> = DatasourceDimensionProps<T> & {
    ...
    core: Pick<CoreStart, 'http' | 'notifications' | 'uiSettings' | 'overlays' | 'analytics' | 'i18n' | 'theme' | 'userProfile' | 'docLinks'>;
    dateRange: DateRange;
...
export interface ILensInterpreterRenderHandlers extends IInterpreterRenderHandlers {
    event: (event: ClickTriggerEvent | BrushTriggerEvent | LensEditEvent<LensEditSupportedActions> | LensTableRowContextMenuEvent | ChartSizeEvent) => void;
}
...
export type GetCompatibleCellValueActions = (data: CellValueContext['data']) => Promise<LensCellValueAction[]>;
 ...
 */
export type GeneralDatasourceStates = Record<string, GeneralDatasourceState> | StructuredDatasourceStates;
