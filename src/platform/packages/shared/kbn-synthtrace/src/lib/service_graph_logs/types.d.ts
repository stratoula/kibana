import type { InfraCache, InfraDatabase, InfraMessageQueue, InfraDependency, OS_OPTIONS, Protocols, Runtime } from './constants';
...
export type ServiceDependenciesOf<T extends ServiceGraph> = T['services'][number]['infraDeps'][number];
...
export type InfraErrorType = 'k8s_oom' | 'k8s_crash_loop_backoff' | 'message_queue_failure' | 'cache_failure';
...
export declare const isInfraErrorType: (e: ErrorType) => e is InfraErrorType;
/** Canonical HTTP status code for each error type. */
export declare const ERROR_TYPE_STATUS: Record<ErrorType, number>;
...
export type InfraPool<TConditions extends string = never> = {
    healthy: string[];
} & Record<TConditions, ConditionPool>;
...
export interface FailureMap<TServiceName extends string = string, TDepName extends string = string> {
    services?: Partial<Record<TServiceName, ServiceFailure>>;
    infra?: Partial<Record<TDepName, ServiceFailure>>;
}
export type FailuresOrFn<TServiceName extends string = string, TDepName extends string = string> = FailureMap<TServiceName, TDepName> | ((timestamp: number) => FailureMap<TServiceName, TDepName> | undefined);
