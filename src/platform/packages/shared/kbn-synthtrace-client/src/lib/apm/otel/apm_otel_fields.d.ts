...
type AttributesKeys = Pick<ApmFields, 'timestamp.us' | 'metricset.name' | 'metricset.interval' | 'transaction.id' | 'transaction.type' | 'transaction.name' | 'transaction.duration.us' | 'transaction.result' | 'transaction.sampled' | 'span.id' | 'span.name' | 'span.type' | 'span.subtype' | 'span.duration.us' | 'span.destination.service.resource' | 'service.name' | 'service.target.name' | 'service.target.type' | 'processor.event' | 'event.outcome' | 'event.name' | 'url.full'>;
type MetricsKeys = Pick<ApmFields, 'transaction.duration.histogram' | 'span.destination.service.response_time.count' | 'span.destination.service.response_time.sum.us'>;
type ResourceAttributesKeys = Pick<ApmFields, 'agent.name' | 'agent.version' | 'service.name' | 'service.node.name' | 'host.name' | 'container.id' | 'cloud.provider' | 'cloud.region' | 'cloud.availability_zone' | 'cloud.service.name' | 'cloud.account.id' | 'cloud.account.name' | 'cloud.project.id' | 'cloud.project.name' | 'cloud.machine.type' | 'faas.coldstart' | 'faas.id' | 'faas.trigger.type' | 'faas.name' | 'faas.version'>;
...
export type ApmOtelFields = Fields & Partial<{
...
} & ApmOtelAttributes & MetricsAttributes & ResourceAttributes>;
export {};
