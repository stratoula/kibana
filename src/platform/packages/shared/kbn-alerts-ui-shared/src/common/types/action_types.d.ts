...
export type PreConfiguredActionConnector = Omit<ActionConnectorProps<never, never>, 'config' | 'secrets'> & {
    isPreconfigured: true;
    isSystemAction: false;
};
export type UserConfiguredActionConnector<Config, Secrets> = ActionConnectorProps<Config, Secrets> & {
    isPreconfigured: false;
    isSystemAction: false;
};
export type ActionConnector<Config = Record<string, unknown>, Secrets = Record<string, unknown>> = PreConfiguredActionConnector | SystemAction | UserConfiguredActionConnector<Config, Secrets>;
export declare enum ActionConnectorMode {
    Test = "test",
    ActionForm = "actionForm"
}
export type ConnectorFormSchema<Config = Record<string, unknown>, Secrets = Record<string, unknown>> = Pick<UserConfiguredActionConnector<Config, Secrets>, 'actionTypeId' | 'isDeprecated' | 'config' | 'secrets'> & Partial<Pick<UserConfiguredActionConnector<Config, Secrets>, 'id' | 'name' | 'authMode'>>;
...
export interface CustomConnectorSelectionItem {
    ...
    getComponent: (actionConnector: ActionConnector) => React.LazyExoticComponent<ComponentType<{
        actionConnector: ActionConnector;
    }>> | undefined;
}
export interface ActionTypeModel<ActionConfig = any, ActionSecrets = any, ActionParams = any> {
    ...
    validateParams: (actionParams: ActionParams, connectorConfig: ActionConfig | null) => Promise<GenericValidationResult<Partial<ActionParams> | unknown>>;
    actionConnectorFields: React.LazyExoticComponent<ComponentType<ActionConnectorFieldsProps>> | null;
    actionParamsFields: React.LazyExoticComponent<ComponentType<ActionParamsProps<ActionParams>>>;
    actionReadOnlyExtraComponent?: React.LazyExoticComponent<ComponentType<ActionReadOnlyElementProps>>;
...
export type ActionTypeRegistryContract<Connector = unknown, Params = unknown> = PublicMethodsOf<TypeRegistry<ActionTypeModel<Connector, Params>>>;
