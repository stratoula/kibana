import type { JSX, ComponentClass, ComponentProps, ComponentType, Dispatch, FC, Key, MutableRefObject, ReactNode, RefAttributes } from 'react';
import type { AlertConsumers, ALERT_CASE_IDS, ALERT_STATUS, ALERT_MAINTENANCE_WINDOW_IDS } from '@kbn/rule-data-utils';
import type { HttpStart } from '@kbn/core-http-browser';
import type { EsQuerySnapshot, LegacyField } from '@kbn/alerting-types';
import type { EuiDataGridColumn, EuiDataGridColumnCellAction, EuiDataGridControlColumn, EuiDataGridOnColumnResizeHandler, EuiDataGridProps, EuiDataGridRefProps, EuiDataGridSorting, EuiDataGridToolBarVisibilityOptions } from '@elastic/eui';
import type { MappingRuntimeFields, QueryDslQueryContainer } from '@elastic/elasticsearch/lib/api/types';
...
type MergeProps<T, AP> = T extends (args: infer Props) => unknown ? (args: Props & AP) => ReactNode : T extends ComponentClass<infer Props> ? ComponentClass<Props & AP> : never;
...
export interface AlertsTableProps<AC extends AdditionalContext = AdditionalContext> extends PublicAlertsDataGridProps {
    /**
     ...
     */
    renderCellValue?: MergeProps<EuiDataGridProps['renderCellValue'], RenderContext<AC> & AlertWithLegacyFormats>;
    /**
     * Cell popover render function
     */
    renderCellPopover?: MergeProps<EuiDataGridProps['renderCellPopover'], RenderContext<AC> & {
        alert: Alert;
    }>;
    /**
     * Actions cell render function
     */
    renderActionsCell?: MergeProps<EuiDataGridControlColumn['rowCellRender'], RenderContext<AC> & AlertWithLegacyFormats & {
     ...
     * ```
     */
    renderExpandedAlertView?: ComponentType<Omit<RenderContext<AC>, 'expandedAlertIndex'> & {
 ...
 */
export type GetAlertsTableProp<PropKey extends keyof AlertsTableProps> = NonNullable<AlertsTableProps[PropKey]>;
export interface AlertsTableImperativeApi {
    refresh: () => void;
    toggleColumn: (columnId: string) => void;
}
export type AlertsTablePropsWithRef<AC extends AdditionalContext> = AlertsTableProps<AC> & RefAttributes<AlertsTableImperativeApi>;
...
} & SetRequired<Pick<AlertsTableProps<AC>, 'columns' | 'pageIndex' | 'onPageIndexChange' | 'pageSize' | 'onPageSizeChange' | 'renderCellValue' | 'renderCellPopover' | 'renderActionsCell' | 'expandedAlertIndex' | 'onExpandedAlertIndexChange' | 'renderExpandedAlertView' | 'services' | 'casesConfiguration' | 'openLinksInNewTab' | 'isMutedAlertsEnabled' | 'getAlertFormatter' | 'alertDetailsNavigation'>, 'columns' | 'pageIndex' | 'onPageIndexChange' | 'pageSize' | 'onPageSizeChange' | 'openLinksInNewTab' | 'expandedAlertIndex' | 'onExpandedAlertIndexChange' | 'isMutedAlertsEnabled'> & AC;
...
export interface PublicAlertsDataGridProps extends Omit<EuiDataGridProps, 'renderCellPopover' | 'renderCellValue' | 'aria-labelledby' | 'columnVisibility' | 'rowCount' | 'sorting' | 'cellContext' | 'pagination' | 'columns'> {
    ruleTypeIds: string[];
    minScore?: number;
    trackScores?: boolean;
    consumers?: string[];
...
export interface AlertsDataGridProps<AC extends AdditionalContext = AdditionalContext> extends PublicAlertsDataGridProps, Pick<EuiDataGridProps, 'columnVisibility'> {
...
export type AlertActionsProps<AC extends AdditionalContext = AdditionalContext> = RenderContext<AC> & EuiDataGridCellValueElementProps & {
    key?: Key;
    alert: Alert;
...
export interface BulkActionsConfig {
    ...
    onClick?: (selectedIds: TimelineItem[], isAllSelected: boolean, setIsBulkActionsLoading: (isLoading: boolean) => void, clearSelection: () => void, refresh: () => void) => void;
...
export declare enum BulkActionsVerbs {
    add = "add",
    delete = "delete",
    clear = "clear",
...
export declare enum AlertsField {
