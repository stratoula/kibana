import type { AGENT_TYPE_EPHEMERAL, AGENT_TYPE_PERMANENT, AGENT_TYPE_TEMPORARY, FleetServerAgentComponentStatuses, AgentStatuses, AGENT_TYPE_OPAMP } from '../../constants';
import type { SecretReference, SOSecret } from './secret';
export type AgentType = typeof AGENT_TYPE_EPHEMERAL | typeof AGENT_TYPE_PERMANENT | typeof AGENT_TYPE_TEMPORARY | typeof AGENT_TYPE_OPAMP;
type AgentStatusTuple = typeof AgentStatuses;
export type AgentStatus = AgentStatusTuple[number];
export type SimplifiedAgentStatus = 'healthy' | 'unhealthy' | 'orphaned' | 'updating' | 'offline' | 'inactive' | 'unenrolled' | 'uninstalled';
export type AgentActionType = 'UNENROLL' | 'UPGRADE' | 'ROLLBACK' | 'SETTINGS' | 'POLICY_REASSIGN' | 'CANCEL' | 'FORCE_UNENROLL' | 'UPDATE_TAGS' | 'REQUEST_DIAGNOSTICS' | 'POLICY_CHANGE' | 'INPUT_ACTION' | 'MIGRATE' | 'PRIVILEGE_LEVEL_CHANGE';
export type AgentUpgradeStateType = 'UPG_REQUESTED' | 'UPG_SCHEDULED' | 'UPG_DOWNLOADING' | 'UPG_EXTRACTING' | 'UPG_REPLACING' | 'UPG_RESTARTING' | 'UPG_WATCHING' | 'UPG_ROLLBACK' | 'UPG_FAILED';
...
export declare enum UnhealthyReason {
    INPUT = "input",
    OUTPUT = "output",
    OTHER = "other"
}
