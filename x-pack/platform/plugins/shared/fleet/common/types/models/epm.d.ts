...
import type { ASSETS_SAVED_OBJECT_TYPE, agentAssetTypes, dataTypes, monitoringTypes, installationStatuses } from '../../constants';
import type { CloudConnectors, ValueOf } from '..';
import type { PackageSpecManifest, PackageSpecIcon, PackageSpecCategory, RegistryVarGroup } from './package_spec';
...
export type AssetType = KibanaAssetType | ElasticsearchAssetType | ValueOf<AgentAssetType> | DocAssetType;
export declare enum KibanaAssetType {
    dashboard = "dashboard",
    lens = "lens",
...
export declare enum KibanaSavedObjectType {
    dashboard = "dashboard",
    lens = "lens",
    visualization = "visualization",
    search = "search",
...
export declare enum ElasticsearchAssetType {
    index = "index",
...
export type FleetElasticsearchAssetType = Exclude<ElasticsearchAssetType, ElasticsearchAssetType.index>;
...
export type PackagePolicyAssetsMap = AssetsMap & {
    __brand: 'PackagePolicyAssetsMap';
};
...
export interface ArchiveIterator {
    traverseEntries: (onEntry: (entry: ArchiveEntry) => Promise<void>, readBuffer?: (path: string) => boolean) => Promise<void>;
...
export type ArchivePackage = PackageSpecManifest & Pick<RegistryPackage, 'readme' | 'assets' | 'data_streams' | 'internal' | 'elasticsearch'>;
export interface BundledPackage {
    name: string;
    version: string;
    getBuffer: () => Promise<Buffer>;
}
export type RegistryPackage = PackageSpecManifest & Partial<RegistryOverridesToOptional> & RegistryAdditionalProperties & RegistryOverridePropertyValue;
...
export interface DeprecationInfo {
    description: string;
    since?: string;
    replaced_by?: Partial<Record<'package' | 'policyTemplate' | 'input' | 'dataStream' | 'variable', string>>;
}
export declare enum RegistryPolicyTemplateKeys {
...
export type RegistryPolicyTemplate = RegistryPolicyIntegrationTemplate | RegistryPolicyInputOnlyTemplate;
export declare enum RegistryInputKeys {
    name = "name",
    type = "type",
    title = "title",
    description = "description",
...
export declare enum RegistryStreamKeys {
    input = "input",
    title = "title",
    description = "description",
    enabled = "enabled",
...
export type RegistryStreamWithDataStream = RegistryStream & {
    data_stream: RegistryDataStream;
};
export type RegistrySearchResults = RegistrySearchResult[];
export type RegistrySearchResult = Pick<RegistryPackage, 'name' | 'title' | 'version' | 'release' | 'description' | 'type' | 'download' | 'path' | 'icons' | 'internal' | 'data_streams' | 'policy_templates_behavior' | 'policy_templates' | 'categories' | 'discovery' | 'deprecated'>;
...
export type AssetsGroupedByServiceByType = Record<Extract<ServiceName, 'kibana'>, KibanaAssetTypeToParts | undefined> & Record<Extract<ServiceName, 'elasticsearch'>, ElasticsearchAssetTypeToParts | undefined>;
...
export type ElasticsearchAssetTypeToParts = Record<FleetElasticsearchAssetType, ElasticsearchAssetParts[]>;
export declare enum RegistryDataStreamKeys {
    type = "type",
...
export type InputOnlyRegistryDataStream = Omit<Pick<RegistryDataStream, RegistryDataStreamKeys>, RegistryDataStreamKeys.type> & {
    [RegistryDataStreamKeys.type]?: string;
};
...
export type RegistryVarType = 'integer' | 'bool' | 'password' | 'select' | 'text' | 'yaml' | 'string' | 'textarea' | 'duration' | 'url' | 'section_header';
export declare enum RegistryVarsEntryKeys {
    name = "name",
    title = "title",
    description = "description",
    type = "type",
...
export interface RegistryVarsEntry {
    ...
    [RegistryVarsEntryKeys.options]?: Array<{
        value: string;
        text: string;
    }>;
...
export type InstallationInfo = {
    ...
    updated_at?: string;
    namespaces?: string[];
} & Omit<Installation, 'package_assets' | 'es_index_patterns' | 'install_version' | 'install_started_at' | 'internal' | 'removable'>;
/**
 * @deprecated
 */
...
export type PackageInfo = Installable<Merge<RegistryPackage, EpmPackageAdditions>> | Installable<Merge<ArchivePackage, EpmPackageAdditions>>;
...
export type ExperimentalIndexingFeature = 'synthetic_source' | 'tsdb' | 'doc_value_only_numeric' | 'doc_value_only_other';
...
export declare enum INSTALL_STATES {
...
export interface Installation {
    ...
    latest_custom_asset_install_failed_attempts?: {
        [asset: string]: CustomAssetFailedAttempt;
    };
...
export type Installable<T> = InstallStatusExcluded<T> | InstalledRegistry<T> | Installing<T> | NotInstalled<T> | InstallFailed<T>;
