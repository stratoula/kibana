...
- Use \`source: "request"\` to create or edit a Lens, Vega, or custom content panel from a natural-language / ES|QL query — this is the only correct way to make a **new** visualization. Never hand-build a visualization \`config\` for a new visualization.
...
4. **Custom content** (\`source: "request"\`, \`renderer: "custom_content"\`) — a LAST RESORT for content that is not a chart at all: bespoke HTML layouts such as KPI scorecards with colored status badges, health/status boards, or panels that mix narrative text with live data values. NEVER use it for anything Lens or Vega can render.
...
Create it like any other visualization panel: \`source: "request"\`, \`renderer: "custom_content"\`, with a natural-language \`query\` describing what to display. The tool generates the data query and the HTML template server-side. Set \`contentMode: "static"\` only when the user explicitly asks for fixed content with no data. Edit it like any other visualization panel: \`edit_panels\` with \`source: "request"\` — the panel keeps its renderer and existing content is used as the baseline.
...
- If a user wants to change a dashboard panel's content, prefer \`edit_panels\` over removing and re-adding the panel. \`edit_panels\` works for ES|QL-backed Lens, Vega, and custom content visualization panels (\`source: "request"\`) and markdown panels (\`source: "config"\`, \`type: "markdown"\`).
