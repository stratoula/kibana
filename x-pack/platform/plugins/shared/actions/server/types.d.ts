import type { CustomRequestHandlerContext, ElasticsearchClient, ISavedObjectsRepository, IScopedClusterClient, KibanaRequest, Logger, SavedObjectAttributes, SavedObjectsClientContract } from '@kbn/core/server';
...
export interface ActionTypeExecutorOptions<Config extends Record<string, unknown>, Secrets extends Record<string, unknown>, Params> {
    actionId: string;
...
export interface InMemoryConnector<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets> extends ActionResult {
    secrets: Secrets;
    config: Config;
...
export type ExecutorType<Config extends Record<string, unknown>, Secrets extends Record<string, unknown>, Params, ResultData> = (options: ActionTypeExecutorOptions<Config, Secrets, Params>) => Promise<ActionTypeExecutorResult<ResultData>>;
...
export type RenderParameterTemplates<Params extends ActionTypeParams> = (logger: Logger, params: Params, variables: Record<string, unknown>, actionId?: string) => Params;
export interface PreSaveConnectorHookParams<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets> {
    connectorId: string;
    config: Config;
    secrets: Secrets;
    logger: Logger;
...
export interface PostSaveConnectorHookParams<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets> {
    connectorId: string;
    config: Config;
    secrets: Secrets;
    logger: Logger;
...
export interface PostDeleteConnectorHookParams<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets> {
    connectorId: string;
    config: Config;
    logger: Logger;
    request: KibanaRequest;
...
export type ConnectorLifecyclePostCreateParams = Omit<PostSaveConnectorHookParams, 'secrets' | 'isUpdate'> & {
...
export type ActionType<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets, Params extends ActionTypeParams = ActionTypeParams, ExecutorResultData = void> = ClassicActionType<Config, Secrets, Params, ExecutorResultData> | WorkflowActionType<Config, Secrets, Params, ExecutorResultData>;
export interface ActionTypeCoreFields<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets, Params extends ActionTypeParams = ActionTypeParams> {
    id: string;
    name: string;
    maxAttempts?: number;
...
export type WorkflowActionType<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets, Params extends ActionTypeParams = ActionTypeParams, ExecutorResultData = void> = ActionTypeCoreFields<Config, Secrets, Params> & {
...
export type ClassicActionType<Config extends ActionTypeConfig = ActionTypeConfig, Secrets extends ActionTypeSecrets = ActionTypeSecrets, Params extends ActionTypeParams = ActionTypeParams, ExecutorResultData = void> = ActionTypeCoreFields<Config, Secrets, Params> & {
    executor: ExecutorType<Config, Secrets, Params, ExecutorResultData>;
    validate: {
        params: ValidatorType<Params>;
        config: ValidatorType<Config>;
...
export declare const UNALLOWED_FOR_UNSECURE_EXECUTION_CONNECTOR_TYPE_IDS: string[];
