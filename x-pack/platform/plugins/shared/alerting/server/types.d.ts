...
import type { IRouter, CustomRequestHandlerContext, SavedObjectReference, IUiSettingsClient, KibanaRequest } from '@kbn/core/server';
import type { z } from '@kbn/zod/v4';
import type { DataViewsContract } from '@kbn/data-views-plugin/common';
import type { ENHANCED_ES_SEARCH_STRATEGY, EQL_SEARCH_STRATEGY, ESQL_ASYNC_SEARCH_STRATEGY, EqlRequestParams, ISearchStartSearchSource } from '@kbn/data-plugin/common';
import type { LicenseType } from '@kbn/licensing-types';
import type { IScopedClusterClient, SavedObjectAttributes, SavedObjectsClientContract, Logger } from '@kbn/core/server';
...
import type { RulesSettingsClient, RulesSettingsFlappingClient, RulesSettingsQueryDelayClient } from './rules_settings';
export * from '../common';
import type { Rule, RuleTypeParams, RuleTypeState, ActionGroup, AlertInstanceContext, AlertInstanceState, WithoutReservedActionGroups, ActionVariable, SanitizedRuleConfig, SanitizedRule, RuleAlertData, Artifacts, GapReason } from '../common';
...
export interface RuleExecutorServices<State extends AlertInstanceState = AlertInstanceState, Context extends AlertInstanceContext = AlertInstanceContext, ActionGroupIds extends string = never, AlertData extends RuleAlertData = RuleAlertData> {
    /**
    ...
    getAsyncSearchClient: <T extends AsyncSearchParams>(strategy: AsyncSearchStrategies) => PublicAsyncSearchClient<T>;
}
export type AsyncSearchStrategies = typeof ESQL_ASYNC_SEARCH_STRATEGY | typeof EQL_SEARCH_STRATEGY | typeof ENHANCED_ES_SEARCH_STRATEGY;
...
export interface RuleExecutorOptions<Params extends RuleTypeParams = never, State extends RuleTypeState = never, InstanceState extends AlertInstanceState = never, InstanceContext extends AlertInstanceContext = never, ActionGroupIds extends string = never, AlertData extends RuleAlertData = never> {
    executionId: string;
    logger: Logger;
    params: Params;
...
export interface GetViewInAppRelativeUrlFnOpts<Params extends RuleTypeParams> {
    rule: Pick<SanitizedRule<Params>, 'id'> & Omit<Partial<SanitizedRule<Params>>, 'viewInAppRelativeUrl'>;
    start?: number;
    end?: number;
}
export type GetViewInAppRelativeUrlFn<Params extends RuleTypeParams> = (opts: GetViewInAppRelativeUrlFnOpts<Params>) => string;
interface ComponentTemplateSpec {
    dynamic?: 'strict' | false;
    fieldMap: FieldMap;
    dynamicTemplates?: Array<Record<string, MappingDynamicTemplate>>;
}
export type FormatAlert<AlertData extends RuleAlertData> = (alert: Partial<AlertData>) => Partial<AlertData>;
export declare const DEFAULT_AAD_CONFIG: IRuleTypeAlerts<DefaultAlert>;
...
export interface RuleType<Params extends RuleTypeParams = never, ExtractedParams extends RuleTypeParams = never, State extends RuleTypeState = never, InstanceState extends AlertInstanceState = never, InstanceContext extends AlertInstanceContext = never, ActionGroupIds extends string = never, RecoveryActionGroupId extends string = never, AlertData extends RuleAlertData = never> {
    id: string;
    name: string;
    validate: {
     ...
     */
    WithoutReservedActionGroups<ActionGroupIds, RecoveryActionGroupId>, AlertData>;
    category: string;
    producer: string;
...
export type UntypedRuleType = RuleType<RuleTypeParams, RuleTypeState, AlertInstanceState, AlertInstanceContext>;
...
export type PartialRule<Params extends RuleTypeParams = never> = Pick<Rule<Params>, 'id'> & Partial<Omit<Rule<Params>, 'id'>>;
export interface RuleWithLegacyId<Params extends RuleTypeParams = never> extends Rule<Params> {
    legacyId: string | null;
}
export type SanitizedRuleWithLegacyId<Params extends RuleTypeParams = never> = Omit<RuleWithLegacyId<Params>, 'apiKey'>;
export type PartialRuleWithLegacyId<Params extends RuleTypeParams = never> = Pick<RuleWithLegacyId<Params>, 'id'> & Partial<Omit<RuleWithLegacyId<Params>, 'id'>>;
...
export interface PublicRuleMonitoringService {
    setMetric: <MetricName extends keyof ConsumerExecutionMetrics>(metricName: MetricName, value: ConsumerExecutionMetrics[MetricName]) => void;
...
export type { RawRule, RawRuleAction, RawRuleExecutionStatus, RawRuleAlertsFilter, RawRuleLastRun, RawRuleMonitoring, } from './saved_objects/schemas/raw_rule';
