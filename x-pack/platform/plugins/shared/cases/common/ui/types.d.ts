...
import type { CREATE_CASES_CAPABILITY, DELETE_CASES_CAPABILITY, READ_CASES_CAPABILITY, UPDATE_CASES_CAPABILITY, CREATE_COMMENT_CAPABILITY, CASES_REOPEN_CAPABILITY, ASSIGN_CASE_CAPABILITY, MANAGE_TEMPLATES_CAPABILITY } from '..';
import type { CASES_CONNECTORS_CAPABILITY, CASES_SETTINGS_CAPABILITY, PUSH_CASES_CAPABILITY } from '../constants';
import type { SnakeToCamelCase } from '../types';
import type { CaseSeverity, CaseStatuses, UserAction, Case as CaseSnakeCase, User, ActionConnector, AlertAttachment, Attachment, ExternalReferenceAttachment, PersistableStateAttachment, Configuration, CustomFieldTypes, EventAttachment, UnifiedAttachment, AttachmentV2 } from '../types/domain';
import type { CasePatchRequest, CasesFindResponse, CaseUserActionStatsResponse, GetCaseConnectorsResponse, GetCaseUsersResponse, UserActionFindRequestTypes, UserActionFindResponse, CaseMetricsFeature, CasesMetricsResponse, SingleCaseMetricsResponse, CasesSimilarResponse } from '../types/api';
type DeepRequired<T> = {
...
export declare const UserActionTypeAll: "all";
...
export type CasesConfigurationUI = Pick<SnakeToCamelCase<Configuration>, 'closureType' | 'connector' | 'mappings' | 'customFields' | 'templates' | 'id' | 'version' | 'owner' | 'observableTypes'>;
...
export declare const SORT_ORDER_VALUES: SortOrder[];
 ...
 */
export declare enum SortFieldCase {
    closedAt = "closedAt",
    createdAt = "createdAt",
    updatedAt = "updatedAt",
    severity = "severity",
...
export type UpdateKey = keyof Pick<CasePatchRequest, 'connector' | 'description' | 'status' | 'tags' | 'title' | 'settings' | 'severity' | 'assignees' | 'category' | 'customFields' | 'extended_fields'>;
